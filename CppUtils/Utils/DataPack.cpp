...
DataPack::DataPack(const uint8_t* data, int data_len)
{
	...
	if (data[bufferSize - 1] != (uint8_t)DataPachKey::FileEnd)//错误的结束符
	{
		return;
	}
	while (index < bufferSize - 1)
	{
		...
		case DataPachKey::IdStart:
		{
			...
			index += 2;
			if (data[index + idLen] != (uint8_t)DataPachKey::IdEnd)//错误的结束符
				return;
		...
		case DataPachKey::ValueStart:
		{
			...
			index += 4;
			if (data[index + valueLen] != (uint8_t)DataPachKey::ValueEnd)//错误的结束符
				return;
		...
		case DataPachKey::ChildStart:
		{
			...
			index += 4;
			if (data[index + childLen] != (uint8_t)DataPachKey::ChildEnd)//错误的结束符
				return;
